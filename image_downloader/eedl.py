...
The script can also create custom mosaics by selecting random points within a region and creating 
a mosaic around each point.
...
def get_collection(
    ...
    Throws:
    ValueError: If an invalid sensor is provided.
    ...
    if sensor == "l8":
        ...
        raise ValueError("Invalid sensor. Choose from 'l8', 'l9', or 's2'.")
...
def get_points_in_region(ee_region, num_points, pts_scale, pts_seed):
    """
    Selects random points within a specified region, focusing on land areas.
    Uses MODIS land/water data to filter out water bodies.
...
def make_rectangle(ee_point, h_pt_buffer, v_pt_buffer=None):
    ...
    h_pt_buffer (float): A float value containing the radius in meters to horizontally extend
    rectangle bounds from the point.
...
def get_and_download_url(index):
    ...
    if out_format == "GEOTiff":
        ...
        ext = ".png"
    f_name = args.sensor + "_" + region_name + "_" + str(index).zfill(5) + ext
    r = requests.get(url, stream=True, timeout=60)
    if r.status_code != 200:
        r.raise_for_status()
    with open(os.path.join(out_path, f_name), "wb") as out_file:
        shutil.copyfileobj(r.raw, out_file)
    print("Download", f_name, "done")
...
if not args.custom_mosaics:
    ...
    # Process landsat sensor.
        ...
        collection_size = min(collection_size, max_ims)
