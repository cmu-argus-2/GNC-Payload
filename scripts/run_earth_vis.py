"""
This script is designed to run the EarthImageSimulator class and generate an image
for a given spacecraft position, attitude and camera model.
It requires the following arguments:
    --name: Name of the experiment
    --meas_rate: Rate at which measurements should be taken

The script requires as input the spacecraft trajectory and attitude, which are 
generated by the run_dynamics script and should be stored as: 
- /output_dir
    - /experiment_name
        - ground_truth.npz
            - attitude
...
The script will generate (append in the args.json case) the following contents 
...
def image_vis(args: argparse.Namespace) -> None:
    ...
    if not os.path.exists(f"{output_dir}/ground_truth.npz"):
        raise FileNotFoundError(
            f"One of the required files in {args.name} does not exist."
            + "Please run the trajectory generation script first."
        )
    data: dict[str, np.ndarray] = np.load(f"{output_dir}/ground_truth.npz")
    ...
    try:
        with open(f"{output_dir}/args.json", "r", encoding="utf-8") as jsonfile:
