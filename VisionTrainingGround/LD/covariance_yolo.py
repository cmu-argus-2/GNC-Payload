...
def match_predictions_with_ground_truth(predictions, ground_truths, iou_threshold=0.5):
    ...
    pred_boxes = np.array([[p[2]-p[4]/2, p[3]-p[5]/2, p[2]+p[4]/2, p[3]+p[5]/2] for p in predictions])
    gt_boxes = np.array([[g[1]-g[3]/2, g[2]-g[4]/2, g[1]+g[3]/2, g[2]+g[4]/2] for g in ground_truths])
...
def analyze_landmark_errors(region, data_path, model_path, output_dir="error_analysis", conf_threshold=0.25):
    """
    ...
    with open(os.path.join(region_output_dir, 'region_error_statistics.txt'), 'w') as f:
        ...
        f.write(f"Mean Position Error (x, y): [{region_stats['mean_position_error'][0]:.2f}, {region_stats['mean_position_error'][1]:.2f}] pixels\n")
        f.write(f"Position Error Magnitude: {region_stats['mean_magnitude']:.2f} ± {region_stats['std_magnitude']:.2f} pixels\n")
    ...
    width, height = 2 * np.sqrt(5.991 * eigenvals)  # 5.991 is chi-square value for 95% confidence with 2 DOF
    ellipse = Ellipse(xy=region_stats['mean_position_error'], width=width, height=height, 
                    angle=angle, edgecolor='red', fc='none', lw=2, label='95% Confidence')
...
def compare_regions(all_stats, output_dir):
    ...
    with open(os.path.join(compare_dir, "region_comparison.txt"), "w") as f:
        ...
        for i, region in enumerate(sorted_regions):
            ...
            f.write(f"{i+1}. Region {region}: RMSE = {stat['rmse_total']:.2f} pixels (from {stat['count']} landmarks)\n")
        
        f.write("\n\n")
        
        # Calculate aggregate statistics
        total_landmarks = sum(stats['count'] for stats in all_stats.values())
        weighted_mean_x = sum(stats['mean_position_error'][0] * stats['count'] for stats in all_stats.values()) / total_landmarks
        weighted_mean_y = sum(stats['mean_position_error'][1] * stats['count'] for stats in all_stats.values()) / total_landmarks
        weighted_rmse = np.sqrt(sum(stats['rmse_total']**2 * stats['count'] for stats in all_stats.values()) / total_landmarks)
        ...
        corr = weighted_cov[0, 1] / np.sqrt(weighted_cov[0, 0] * weighted_cov[1, 1]) if weighted_cov[0, 0] * weighted_cov[1, 1] > 0 else 0
        
        # Write aggregate statistics
        f.write("AGGREGATE STATISTICS ACROSS ALL REGIONS:\n")
        f.write("-"*40 + "\n")
        f.write(f"Total Landmarks: {total_landmarks}\n")
        f.write(f"Weighted Mean Position Error: [{weighted_mean_x:.2f}, {weighted_mean_y:.2f}] pixels\n")
        ...
        f.write(f"  Error Anisotropy Ratio: {(axis_lengths[0]/axis_lengths[1] if axis_lengths[1] > 0 else 0):.2f} (major/minor axis)\n\n")
        ...
        for region in sorted_regions:
            ...
            region_corr = cov[0, 1] / np.sqrt(cov[0, 0] * cov[1, 1]) if cov[0, 0] * cov[1, 1] > 0 else 0
            
            # Write region details
            f.write(f"Region {region}:\n")
            f.write(f"  Count: {stat['count']}\n")
            f.write(f"  Mean Position Error (x, y): [{stat['mean_position_error'][0]:.2f}, {stat['mean_position_error'][1]:.2f}] pixels\n")
            ...
            f.write(f"  Principal Error Axes: {axis_lengths_r[0]:.2f} and {axis_lengths_r[1]:.2f} pixels\n")
            f.write(f"  Error Ellipse Orientation: {angle_r:.2f} degrees\n")
            f.write(f"  Error Anisotropy Ratio: {(axis_lengths_r[0]/axis_lengths_r[1] if axis_lengths_r[1] > 0 else 0):.2f}\n\n")
