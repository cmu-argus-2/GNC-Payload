...
class SensorNoiseParams:
    def __init__(self, biasParams: BiasParams, sigma_v: float, scale_factor_error: float, misalignment1: float, misalignment2: float) -> None:
    ...
    def get_random_params(
        biasParams: BiasParams, sigma_v_range: list, scale_factor_error_range: list, misalignment_range: list
    ) -> "SensorNoiseParams":
        """
...
class Sensor:
    ...
    def update(self, clean_signal: np.ndarray, other_signal1: np.ndarray, other_signal2: np.ndarray) -> np.ndarray:
        """
        ...
        return (1 + self.scale_factor_error) * clean_signal + other_signal1 * self.misalignment1 + other_signal2 * self.misalignment2 + self.bias.get_bias() + noise
