...
KEYS=('05V' '09V' '57V' '59G') # Add or remove keys as needed
...
for KEY in "${KEYS[@]}"; do
  ...
  for YOLO_VER in n s m l x; do
    python ./LD/val_yolo.py --data "$CURR_DIR/LD/datasets/${KEY}_dataset/dataset.yaml" --save_dir "$CURR_DIR/LD/runs/${KEY}_yolov8${YOLO_VER}" --region "$KEY" --version "yolov8${YOLO_VER}" --epochs 200
  done
