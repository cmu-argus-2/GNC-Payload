...
def plot_acceleration(time, accel_components, dir_name: str):
    # xyz acceleration components
    fig, ax = plt.subplots(3, 1, figsize=(10, 8), sharex=True)
    component_names = {
        "earth_gravity": "Earth Gravity",
        "j2": "J2 Perturbation",
        "j34": "J3/J4 Perturbation",
        "drag": "Atmospheric Drag",
        "sun_gravity": "Solar Gravity",
        "moon_gravity": "Lunar Gravity",
    }
    ylbls = ["Ax (km/s²)", "Ay (km/s²)", "Az (km/s²)"]
    for j in range(3):
        ax[j].grid(True)
        for _, (key, values) in enumerate(accel_components.items()):
            ax[j].plot(time, values[:, j], label=component_names.get(key, key))
        ax[j].set_title("Acceleration (km/s²)")
        ax[j].set_ylabel(ylbls[j])
        ax[j].legend()
        ax[j].grid(True)
    ax[-1].set_xlabel("Time (s)")
    fig.tight_layout()
    if dir_name:
        fig.savefig(os.path.join(dir_name, "acceleration_components.png"))
        
    # magnitude of each acceleration in log scale
    fig_mag, ax_mag = plt.subplots(figsize=(10, 4))
    for _, (key, values) in enumerate(accel_components.items()):
        acc_mag = np.linalg.norm(values, axis=1)
        ax_mag.semilogy(time, acc_mag, label=component_names.get(key, key))
    ax_mag.set_title("Acceleration Magnitude (km/s²)")
    ax_mag.set_xlabel("Time (s)")
    ax_mag.set_ylabel("Acceleration Magnitude (km/s²)")
    ax_mag.legend()
    ax_mag.grid(True)
    fig_mag.tight_layout()
    if dir_name:
        fig_mag.savefig(os.path.join(dir_name, "acceleration_magnitude.png"))
...
def plot_syn_data(time, states: np.ndarray, ld_meas, gyro_meas, accel_components, dir_name: str):
    if dir_name:
    ...
    # Acceleration figure
    plot_acceleration(time, accel_components, dir_name)
